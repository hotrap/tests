...
overhead = max(ops1KiB['OverheadUniformRocksDBFat1KiB'], ops200B['OverheadUniformRocksDBFat200B'])
...
print('\defmacro{OverheadUniformRocksDBFatCeil}{%d\\%%}' %math.ceil(overhead * 100), file=tex)
tex = tex.getvalue()
print(tex)
open(os.path.join(dir, 'overhead-uniform-rocksdb-fat.tex'), mode='w').write(tex)
