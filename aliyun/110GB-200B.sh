...
function run-hotrap {
	...
	IP=$3
	./checkout-$version $IP
	# Reserve 1.5GB for VisCnts
	ssh root@$IP -o ServerAliveInterval=60 "source ~/.profile && cd tests/workloads && ./test-hotrap-110GB-200B.sh ../config/$workload ../../data/$workload/$version 8.5GB 5.5GB"
	rsync -zPrt -e ssh root@$IP:~/data/$workload $output_dir/
