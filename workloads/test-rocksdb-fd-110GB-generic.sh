...
ulimit -c unlimited
cd $DIR
$workspace/tests/helper/exe-while.sh . bash -c "$prefix $kvexe_dir/rocksdb-kvexe --switches=0x1 --num_threads=16 --max_background_jobs=8 --block_size=16384 --cache_size=134217728 --max_bytes_for_level_base=671088640 --enable_fast_generator --enable_fast_process --workload_file=$workload_file --db_path=$workspace/testdb/db/ --db_paths=\"{{$workspace/testdb/fd,1000000000000}}\" $extra_kvexe_args 2>> log.txt"
