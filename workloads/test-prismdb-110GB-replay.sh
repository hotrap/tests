...
ulimit -c unlimited
cd $DIR
$workspace/tests/helper/exe-while.sh . bash -c "systemd-run --user --scope -p MemoryMax=5G $kvexe_path --load=$trace_file_load --run=$trace_file_run --format=plain-length-only --num_threads=16 --cache_size=134217728 --db_path=$workspace/testdb/sd --db_paths=\"{{$workspace/testdb/fd,$fd_size},{$workspace/testdb/sd,$sd_size}}\" --switches=1 --migrations_logging=1 --read_logging=0 --migration_policy=2 --migration_metric=1 --migration_rand_range_num=8 --migration_rand_range_size=1 --num_keys=$num_keys --optane_threshold=0.1 --slab_dir=$workspace/testdb/fd/slab-%d-%lu-%lu --pop_cache_size=22000000 --read_dominated_threshold=0.95 --stop_upsert_trigger=70000000 2>> log.txt"
