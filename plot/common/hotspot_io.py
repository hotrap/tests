...
def draw_hotspot_io(dir, size, pdf_name):
    ...
    ycsb_configs=['ycsbc', 'read_0.5_insert_0.5']
    cluster_labels = ['i-0%', 'i-50%']
    versions=[
        {
    ...
    plt.xlabel('(a) I/O of warm-up phase', fontsize=8)

    subfig = plt.subplot(gs[0, 1])
    draw_io(warmup_finish_progress, end_progress_fn)
    plt.xlabel('(b) I/O of stable phase', fontsize=8)
    ...
    fig.legend(
        ...
        fontsize=8, ncol=3, loc='center', bbox_to_anchor=(0.5, 1.03)
    )
